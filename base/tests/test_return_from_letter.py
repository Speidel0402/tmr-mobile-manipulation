...
class ReturnFromLetterContracts(unittest.TestCase):
    def test_measured_left_precedes_turn_and_door_child(self) -> None:
        points = [
            SOURCE.index("node.translate(0.0, args.left_m"),
            SOURCE.index("node.rotate_ccw(math.radians(args.turn_ccw_deg)"),
    ...
    def test_resume_door_requires_completed_base_realign(self) -> None:
        ...
        self.assertIn('{"LEFT_BY_MEASURED_OUTBOUND", "TURN_CCW_180"}.issubset(completed)', SOURCE)
