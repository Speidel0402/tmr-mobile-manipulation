...
| Arm controller stops during spine, camera, gripper or base work | Check `arm_publish_count`, `last_published_arm_input`, `last_arm_publish_age_s`, and spine `arm_keepalive_count` | The current runner keeps the encoded GELLO hold stream alive through all loops and while waiting for the spine action/service. Send the JSON if the controller still reports stale input. |
