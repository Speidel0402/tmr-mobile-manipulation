...
class InitialPoseConsistencyTests(unittest.TestCase):
    ...
    def test_base_startup_uses_one_isolated_graph(self) -> None:
        ...
        self.assertIn('start_left_ik_service.py', source)
        self.assertIn('Left FK IK readiness', source)
        self.assertIn("'/left_ik/compute_fk'", source)
        self.assertIn("'/left_ik/compute_cartesian_path'", source)
        self.assertIn('Left wrist snapshot readiness', source)
        self.assertIn('camera_mjpeg_viewer.py', source)
        self.assertIn('http://127.0.0.1:18080/healthz', source)
