...
| Arm controller stops during spine, image processing, IK, evidence writing, gripper or base work | Check `arm_publish_count`, `last_published_arm_input`, `last_arm_publish_age_s`, `arm_keepalive_thread_alive`, `arm_keepalive_error`, and spine `arm_keepalive_count` | A dedicated 50 Hz publisher owns the continuous arm stream so DDS publication cannot stall input callback servicing. Send the JSON if the controller still reports stale input. |
| All inbound samples age together near the feedback-age guard while their source topics remain healthy | Compare `joint_feedback_age_s` with independent topic-rate evidence and the configured `joint_feedback_stale_timeout_s` | The runner drains multiple ready callbacks per control tick and uses a bounded `1.0 s` default. Preserve the JSON; only if measured process-local gaps exceed it, set `TMR_HAMBURG_ARM_JOINT_FEEDBACK_STALE_TIMEOUT_S` to a value supported by that evidence (maximum `5 s`). Do not relax it for a source topic that has actually stopped. |
...
for recovery. The feedback-age guard defaults to `1.0 s`; it is independent of
the trajectory timing and is capped at `5 s` when overridden.
