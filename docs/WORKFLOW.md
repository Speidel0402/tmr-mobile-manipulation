...
分类桌返回物品桌使用 `base/scripts/15_return_from_letter.py`。保留原有的实测距离左移和门框中点复核，只把返回的 `180°` 旋转改为顺时针，避免与去程同向累计而缠绕线缆；之后仍执行门前 `0.50 m` 与前进 `1.20 m`。

综合三物体任务由 `base/scripts/17_control_mode.sh` 管理控制权：任务最前方停止手柄速度节点并锁定任务通道，正常完成、异常或人工中断后的最后一步恢复 Xbox 遥控。它不重启底盘驱动，避免生成第二个控制器或第二个速度发布者。
...
6. 沿地面垂直方向从本轮捕获的顶层高度下降 `0.340 m`。每个动作有超时，最后用实测终点判断；必要时只做一次有限精度收敛。
